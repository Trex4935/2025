...
public class cm_SetCoralIntake extends Command {
  ...
  public boolean isFinished() {
    return (Duration.between(startTime, Instant.now()).toMillis() > 500
            && m_coralIntake.coralIntakeMotor.getVelocity().getValueAsDouble() < 0.5)
        || (Duration.between(startTime, Instant.now()).toMillis() > 2000);
  }
}
